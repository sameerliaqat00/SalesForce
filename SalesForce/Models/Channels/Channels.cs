...
namespace SalesForce.Models.Channels
{
    ...
    public class ChannelsHandler
    {
        ...
        {
            query = "insert into tbl_Channels(ChannelId,ChannelName,Status,SubChannelId)Values('";
            query = query + Channels.ChannelId + "','";
            query = query + Channels.ChannelName + "','";
            query = query + Channels.Status + "','";
            query = query + Channels.SubchannelId + "')";
        ...
        public int Update(Channels Channels)
        {
            ...
            query = query + " Status = '" + Channels.Status + "',";
            query = query + " SubChannelId = '" + Channels.SubchannelId + "'";
        ...
        public Channels GetById(int id)
        {
            ...
            if (Data.Rows.Count > 0)
            {
                ...
                foreach (DataRow dataRow in Data.Rows)
                {
                    ...
                    Channels.Status = Convert.ToBoolean(dataRow["Status"]);
                    Channels.SubchannelId = Convert.ToInt32(dataRow["SubChannelId"]);
                }
        ...
        public int GetMaxId()
        {
            query = "select isnull(max(Channelid),0) + 1 tbl_Channels";
